...
const TaxesList = ({ db, onAddNew, onEdit, onDelete, setTaxes }) => {
  ...
  const handleToggleActive = useCallback(async (id, newStatus) => {
    ...
    try {
      ...
      const docRef = doc(db, "usuarios", user.uid, "impuestos", id);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // ¡CAMBIO! Ruta anidada con user.uid
      const q = query(
        collection(db, "usuarios", user.uid, "impuestos"),
        orderBy("posicion")
      );
      const querySnapshot = await getDocs(q);
      const data = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setItems(data);
      if (setTaxes) setTaxes(data);
    } catch (error) {
      console.error("Error fetching taxes:", error);
    } finally {
      setLoading(false);
    }
  }, [db, user, setTaxes]); // ¡CAMBIO! Añadir 'user' a las dependencias
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        ...
        reorderedItems.forEach((item, index) => {
          // ¡CAMBIO! Ruta anidada con user.uid
          const docRef = doc(db, "usuarios", user.uid, "impuestos", item.id);
